...
logging.basicConfig(
    filename='ssh_public_removal.log',
...
PORT_TO_REMOVE = 22  # SSH port
OUTPUT_CSV = 'public_ssh_security_groups.csv'
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Identify and optionally remove public SSH access from AWS security groups.')
...
def find_security_groups_with_public_ssh(ec2_client):
    """Find security groups that have SSH open to the public."""
    try:
        response = ec2_client.describe_security_groups()
        security_groups_with_public_ssh = []
                ...
                if (permission.get('IpProtocol') == 'tcp' and
                    ...
                    for ip_range in permission.get('IpRanges', []):
                        if ip_range.get('CidrIp') == CIDR_IP_TO_REMOVE:
                            security_groups_with_public_ssh.append(sg)
                            logging.info(f"Security group {sg['GroupId']} in region {ec2_client.meta.region_name} has SSH open to the public.")
                            break  # Break to avoid duplicate alerts for the same security group

        return security_groups_with_public_ssh

    except ClientError as e:
        logging.error(f"Error finding security groups with public SSH in region {ec2_client.meta.region_name}: {e}")
        return []
...
def remove_ssh_inbound_rule(ec2_client, sg_id):
    """Remove the inbound SSH rule from a security group."""
    try:
        ...
        # Send Slack alert
        message = (f":white_check_mark: **Removed Public SSH Access**\n"
        ...
        message = (f":x: **Failed to Remove Public SSH Access**\n"
                   f"**Security Group ID:** `{sg_id}`\n"
                   f"**Region:** `{ec2_client.meta.region_name}`\n"
...
def main():
    ...
    for region in regions:
        ...
        sgs_with_public_ssh = find_security_groups_with_public_ssh(ec2_client)

        for sg in sgs_with_public_ssh:
    ...
    if not all_sgs_details:
        logging.info("No security groups with public SSH access found across all regions.")
        print("No security groups with public SSH access found across all regions.")

        # Send Slack alert for no security groups found
        send_slack_alert(":tada: No security groups with public SSH access were found in the scanned regions.")
        return

    # Record details to CSV
    record_security_group_details(all_sgs_details)
    print(f"Details of security groups with public SSH access have been recorded in '{OUTPUT_CSV}'.")
    logging.info(f"Details recorded in '{OUTPUT_CSV}'.")

    # Send CSV file to Slack
    message = ":page_facing_up: **SSH Public Access Security Groups Report**"
    send_slack_alert(message, OUTPUT_CSV)
    logging.info("Uploaded CSV report to Slack.")

    # Display summary to the user
    print("\nSummary of Security Groups with Public SSH Access:")
    for sg_detail in all_sgs_details:
        sg = sg_detail['SecurityGroup']
        region = sg_detail['Region']
        sg_id = sg['GroupId']
    ...
    if is_dry_run:
        ...
        for sg_detail in all_sgs_details:
            ...
            region = sg_detail['Region']
            sg_id = sg['GroupId']
            print(f"[Dry Run] Would remove public SSH access from Security Group {sg_id} in region {region}.")
            logging.info(f"Dry run: Would remove public SSH access from Security Group {sg_id} in region {region}.")
        return

    # Prompt user for confirmation (only if not dry run)
    user_input = input("\nDo you want to remove the public SSH access from these security groups? (yes/no): ").strip().lower()
    ...
    # Proceed to remove SSH rules
    for sg_detail in all_sgs_details:
        sg = sg_detail['SecurityGroup']
        region = sg_detail['Region']
        sg_id = sg['GroupId']
        ec2_client = boto3.client('ec2', region_name=region)
        success = remove_ssh_inbound_rule(ec2_client, sg_id)
        if success:
            print(f"Successfully removed public SSH access from Security Group {sg_id} in region {region}.")
        else:
            print(f"Failed to remove public SSH access from Security Group {sg_id} in region {region}. Check logs for details.")

    # Send completion Slack alert
    message = (f":information_source: **SSH Public Access Removal Script Completed**\n"
               f"Regions Scanned: `{len(regions)}`\n"
               f"Security Groups Modified: `{len(all_sgs_details)}`")
    send_slack_alert(message)

    print("\nOperation completed. Check 'ssh_public_removal.log' for detailed logs.")
    logging.info("Completed removal of public SSH access from identified security groups.")
